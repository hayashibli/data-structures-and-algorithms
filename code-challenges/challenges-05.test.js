...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(item => {
    ...
    template.html(`<h2>${item.name}</h2><br><h3>${item.height}</h3><br><p>${item.eye_color}</p>`)
...
let listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  let regex = /[0-9]+\s\b\S+\b\s/g;
    recipe.ingredients.forEach(listItems => {
      let index = listItems.match(regex)[0].length;
      let ingredient = listItems.slice(index);
